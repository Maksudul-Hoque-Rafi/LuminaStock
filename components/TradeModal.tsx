...
export const TradeModal: React.FC<TradeModalProps> = ({ isOpen, onClose, symbol, currentPrice, cashBalance, onTrade }) => {
    ...
    <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-lg max-w-sm w-full p-6 relative">
        ...
        <div className="bg-slate-50 p-3 rounded-lg mb-4 flex items-center justify-between">
          ...
          <span className="font-bold text-slate-900">${cashBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>

        {/* Type Toggle */}
        <div className="flex gap-2 mb-4 p-1 bg-slate-100 rounded-lg">
            <button 
                className={`flex-1 py-1.5 rounded-md text-sm font-semibold transition-all ${type === 'buy' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                onClick={() => setType('buy')}
            >
                Buy
            </button>
            <button 
                className={`flex-1 py-1.5 rounded-md text-sm font-semibold transition-all ${type === 'sell' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
        ...
        <div className="space-y-4 mb-6">
            ...
            <div className={`p-3 rounded-lg space-y-2 border ${!canAfford && type === 'buy' ? 'bg-rose-50 border-rose-100' : 'bg-slate-50 border-transparent'}`}>
                ...
                <div className="flex justify-between text-sm font-bold text-slate-900 pt-2 border-t border-slate-200/50">
                    ...
                    <span className={!canAfford && type === 'buy' ? 'text-rose-600' : ''}>
        ...
        <button 
            ...
            className={`w-full py-3 rounded-lg font-bold text-white shadow-sm transition-colors 
              ${type === 'buy' 
                ? (canAfford ? 'bg-blue-600 hover:bg-blue-700' : 'bg-slate-300 cursor-not-allowed') 
