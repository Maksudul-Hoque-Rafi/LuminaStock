...
import { Star, TrendingUp, DollarSign, Activity, AlertCircle, Cpu, Newspaper, Scale, Clock } from 'lucide-react';
import { getStock, MOCK_STOCKS } from '../services/mockData';
import { getAIStockAnalysis, getAIStockNews } from '../services/geminiService';
import { executeTrade, getHolding, getCashBalance } from '../services/portfolioService';
...
const StockDetails: React.FC = () => {
  ...
  const generateChartData = () => {
    const data = [];
    let base = stock?.price || 150;
    for (let i = 0; i < 30; i++) {
      base = base * (1 + (Math.random() - 0.5) * 0.05);
      data.push({ day: i + 1, price: base });
    }
    return data;
  };
    ...
    try {
      ...
      if (result && result.length > 0) {
        setStockNews(result);
         ...
         setStockNews([
          { title: `${symbol} Market Movement`, summary: "Latest trading data shows active volume.", source: "MarketWatch", time: "Today", url: `https://www.google.com/search?q=${symbol}+stock+news` }
        ]);
      }
    } catch {
      setStockNews([]);
  ...
  if (!stock) {
    return <div className="text-center py-20 text-slate-500">Stock not found or loading...</div>;
  }

  const isPositive = stock.change >= 0;
  const peerStocks = MOCK_STOCKS.filter(s => s.sector === stock.sector && s.symbol !== stock.symbol).slice(0, 3);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="px-2 py-1 bg-slate-100 text-slate-600 text-xs rounded-full font-medium">{stock.sector}</span>
          </div>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-bold text-slate-900">${stock.price.toFixed(2)}</span>
            <span className={`text-lg font-medium mb-1 ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button 
            ...
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg border font-medium transition-colors ${inWatchlist ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
           ...
           {/* About / Description */}
           <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
             <h3 className="text-lg font-bold text-slate-900 mb-3">About {stock.name}</h3>
                     ...
                     {peerStocks.map(peer => (
                       <tr key={peer.symbol}>
                         <td className="py-3 font-medium text-slate-900">
                           <Link to={`/stock/${peer.symbol}`} className="hover:text-blue-600">{peer.name} ({peer.symbol})</Link>
                         </td>
                         <td className="py-3 text-right">${peer.price.toFixed(2)}</td>
                         <td className="py-3 text-right text-slate-600">{peer.marketCap}</td>
                         <td className="py-3 text-right text-slate-600">{peer.peRatio}</td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
        ...
        <div className="space-y-6">
           ...
           {currentHolding && (
              <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
                 <h3 className="text-sm font-bold text-blue-900 uppercase tracking-wide mb-3">Your Position</h3>
                 <div className="flex justify-between items-end mb-2">
                    <span className="text-slate-600">Shares</span>
                    <span className="text-xl font-bold text-slate-900">{currentHolding.quantity}</span>
                 </div>
                 <div className="flex justify-between items-end">
                    <span className="text-slate-600">Avg Cost</span>
                    <span className="font-medium text-slate-900">${currentHolding.avgBuyPrice.toFixed(2)}</span>
                 </div>
              </div>
           )}
           ...
           <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
             ...
             <div className="space-y-4 text-sm">
                <div className="flex justify-between border-b border-slate-50 pb-2">
                   ...
                   <span className="font-medium text-slate-900">{stock.marketCap}</span>
                </div>
                <div className="flex justify-between border-b border-slate-50 pb-2">
           ...
           <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-xl border border-indigo-100">
             ...
             {!aiAnalysis && !loadingAnalysis && (
               <div className="text-center py-4">
                  <p className="text-sm text-indigo-700 mb-3">Get a quick AI-powered breakdown of this stock's potential.</p>
                  <button 
             ...
             {aiAnalysis && (
               <div className="prose prose-sm prose-indigo text-slate-700 mt-2">
                 <div className="whitespace-pre-line leading-relaxed text-sm">{aiAnalysis}</div>
               </div>
             )}
           </div>
             ...
             <div className="space-y-4">
               {loadingNews ? (
                 ...
                 stockNews.map((news, i) => (
                   <a 
                      key={i} 
                      href={news.url || `https://www.google.com/search?q=${encodeURIComponent(news.title)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group cursor-pointer block"
                   >
                      <h4 className="text-sm font-semibold text-slate-900 group-hover:text-blue-600 mb-1 leading-snug hover:underline">{news.title}</h4>
                      <div className="flex items-center gap-2 text-xs text-slate-400">
                        <span>{news.source}</span>
                        <span>•</span>
                        <span className="flex items-center gap-0.5"><Clock size={10} /> {news.time}</span>
                      </div>
                   </a>
                 ))
               ) : (
                 <div className="text-sm text-slate-500">No recent news available.</div>
               )}
             </div>
           </div>
        </div>
