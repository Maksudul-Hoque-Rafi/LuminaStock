...
const Watchlist: React.FC = () => {
  ...
  useEffect(() => {
    const tickers: string[] = JSON.parse(localStorage.getItem('watchlist') || '[]');
    const stocks = tickers.map(t => MOCK_STOCKS.find(s => s.symbol === t)).filter((s): s is Stock => !!s);
    setWatchlistStocks(stocks);
  }, []);

  const removeFromWatchlist = (symbol: string) => {
    const currentTickers = JSON.parse(localStorage.getItem('watchlist') || '[]');
    const newTickers = currentTickers.filter((t: string) => t !== symbol);
  ...
  if (watchlistStocks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-slate-500 mb-6">Keep track of your favorite stocks by adding them here.</p>
        <Link to="/screener" className="text-blue-600 font-medium hover:underline">
          Go to Screener
        </Link>
      </div>
    );
